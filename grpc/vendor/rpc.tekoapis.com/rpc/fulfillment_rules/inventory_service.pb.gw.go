...
var (
	filter_InventoryService_GetInventory_0 = &utilities.DoubleArray{Encoding: map[string]int{}, Base: []int(nil), Check: []int(nil)}
)
...
func request_InventoryService_GetInventory_0(ctx context.Context, marshaler runtime.Marshaler, client InventoryServiceClient, req *http.Request, pathParams map[string]string) (proto.Message, runtime.ServerMetadata, error) {
	...
	if err := runtime.PopulateQueryParameters(&protoReq, req.Form, filter_InventoryService_GetInventory_0); err != nil {
		return nil, metadata, status.Errorf(codes.InvalidArgument, "%v", err)
	}
...
// Note that using this registration option will cause many gRPC library features (such as grpc.SendHeader, etc) to stop working. Consider using RegisterInventoryServiceHandlerFromEndpoint instead.
...
var (
	pattern_InventoryService_GetInventory_0 = runtime.MustPattern(runtime.NewPattern(1, []int{2, 0}, []string{"inventory"}, "", runtime.AssumeColonVerbOpt(true)))
)
